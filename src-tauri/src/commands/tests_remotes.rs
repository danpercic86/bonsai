...
fn remote_management_crud_and_failures() {
    ...
    block_on(add_remote_inner(
        &state,
        &id,
        "origin".into(),
        "https://example.com/a.git".into(),
    ))
    .expect("add origin");
    ...
    let err = block_on(add_remote_inner(
        &state,
        &id,
        "origin".into(),
        "https://example.com/b.git".into(),
    ))
    ...
    block_on(set_remote_url_inner(
        ...
        &id,
        "origin".into(),
        "https://example.com/c.git".into(),
    ))
    .expect("set url");
    let url = block_on(list_remotes_inner(&state, &id))
        .unwrap()
        .into_iter()
    ...
    let err = block_on(set_remote_url_inner(
        &state,
        &id,
        "ghost".into(),
        "https://x/y.git".into(),
    ...
    .expect("rename");
    assert!(block_on(list_remotes_inner(&state, &id))
        .unwrap()
        .iter()
    ...
    assert!(block_on(list_remotes_inner(&state, &id))
        ...
        .is_empty());
    let err =
        block_on(remove_remote_inner(&state, &id, "upstream".into())).expect_err("remove missing");
    assert!(matches!(err, AppError::NoRemote(_)), "{err:?}");
}
...
fn pushed_local(state: &AppState) -> (tempfile::TempDir, String, tempfile::TempDir, String) {
    ...
    match block_on(push_inner(state, &id, None)).expect("initial push") {
        PushResult::Pushed { set_upstream, .. } => {
            assert!(set_upstream, "first push sets upstream")
        }
...
fn pull_ff_diverged_and_no_upstream() {
    ...
    match block_on(pull_inner(&state, &id2)).expect("pull diverged") {
        PullResult::WouldNotFastForward { ahead, behind, .. } => {
            assert!(
                ahead > 0 && behind > 0,
                "genuinely diverged: ahead={ahead} behind={behind}"
            );
        }
