...
//!
//! …plus the metrics commands (§8). `logs_delete_all` (§6.1 / §F6) is the one
//! command here that destroys anything, so it lives in `obs_delete.rs`.
//!
...
/// work), and §F6 deliberately did NOT add one: `logs_delete_all` is the user's
/// door, and a "clear usage statistics" control belongs on the Statistics page,
/// next to the data. Blocking (writes the file), so it runs on the blocking pool.
///
/// §F6 — takes `AppState` for the PERF SNAPSHOT: `reset` re-baselines
/// `perf_baseline` to the current counters, because `PerfState` is
/// process-lifetime and zeroing the baseline resurrects every pre-reset count at
/// the next fold.
#[tauri::command]
pub async fn metrics_reset(
    ...
    let metrics = Arc::clone(&metrics);
    tauri::async_runtime::spawn_blocking(move || {
        metrics.reset(&perf.snapshot(), writer::now_secs())
    })
    .await
    .map_err(|e| AppError::Other(format!("task join error: {e}")))?
}
