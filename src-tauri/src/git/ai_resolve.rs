...
/// Rendered for an index stage that has no entry (contract §5.1). (P13)
...
/// - Builds the stdin payload (§5.1), calls `ai::run_claude` with the system
...
pub fn ai_resolve_conflict(
    ...
    // Labeled-section stdin payload (contract §5.1).
    let payload = format!(
        "FILE: {path}\nCONFLICT KIND: {kind:?}\n\n\
         ===== ANCESTOR (base) =====\n{base}\n\n\
         ===== OURS =====\n{ours}\n\n\
