...
| `obs/metrics_keys.rs` | **The metrics key privacy guard.** Sole home of the `<domain>.<action>` / `cmd.<name>` / error-code shape predicates that decide whether a string may become a key in `usage.json`. Separate from `metrics.rs` **by contract, not by size**: `usage.json` is durable and has no redaction pass, so a user-derived key there is long-lived repo content. **SUPERSEDED in part 2026-09-11 (F6 user ruling, see `P91-F6-usage-retention.md`): this row previously said `usage.json` "is not covered by `logs_delete_all`" and is "permanent, un-deletable". Both are now FALSE — `logs_delete_all` clears the `metrics` folder and resets the in-memory state, and the per-day profile is pruned at 90 days. The key guard still matters: a bad key survives until a delete or the 90-day fold, and `first_seen`/`sessions` are lifetime.** |
...
<app_config_dir>/            # sibling to settings.json (com.bonsai.app)
  ...
  exports/                                           # §6.2 — default target for log_export_session
                                                     # SUPERSEDED — see
                                                     # docs/contracts/P91-F6-usage-retention.md §2:
                                                     # the ONLY target; there is no other
...
**SUPERSEDED — see docs/contracts/P91-F6-usage-retention.md §5.** The command and its no-UI status
survive; the parenthetical's reasoning does not.
...
**SUPERSEDED — see docs/contracts/P91-F6-usage-retention.md §3.** (`MetricsSnapshot.days` above,
"retained: 400 days" — 90 days.)
...
- **Key namespace — three independent gates, see §8.2.** Key *names* are enumerated in
  ...
  by `logs_delete_all`.
  **SUPERSEDED — see docs/contracts/P91-F6-usage-retention.md §4.** They are covered. The *other*
  half of that sentence (metrics structurally cannot contain repo content ⇒ no redaction pass) is
  **unchanged and still ratified**.
...
**SUPERSEDED — see docs/contracts/P91-F6-usage-retention.md §3.5.** ("400 days ≈ 1.2 MB worst case"
— recomputed at 90.)
...
**SUPERSEDED — see docs/contracts/P91-F6-usage-retention.md §3.5.** (G3's "400-day→`lifetime`
roll-up".)
...
**SUPERSEDED — see docs/contracts/P91-F6-usage-retention.md §3.5.** (The corrected ceiling, "≈ 4.3 MB
over 400 days".)
...
durable. **SUPERSEDED 2026-09-11 (F6 user ruling, see `P91-F6-usage-retention.md`): the rest of this
sentence previously read "is outside the `logs_delete_all` scope (§8), and `metrics_reset` is the
user's only way to clear it." Both clauses are now FALSE — `logs_delete_all` deletes every file in
the `metrics` folder, then the folder, AND resets the in-memory `MetricsState`; `metrics_reset`
remains a distinct metrics-only primitive sharing the same `clear`. The defect this paragraph
describes is exactly why the in-memory reset is inseparable from the file deletion: clearing only
the file lets the next flush write the deleted bytes straight back.**
...
| `dev.delete-logs` | button (destructive) | **§6.1/§6.2** — confirm dialog first, stating `totalFiles` / `totalBytes` **and `exportFiles` / `exportBytes`** from `log_session_info`; then `logs_delete_all()`. Reports the result honestly: success count + bytes (naming exports separately via `deletedExports`), and a warning state when `failedFiles > 0`. When Dev mode is ON, the copy states that logging continues into a new, empty file. **SUPERSEDED — see docs/contracts/P91-F6-usage-retention.md §4.3.** Scope is logs + exports + the metrics folder. (§10's "No row for `metrics_reset` in P91" is still correct.) |
...
| 4 | Log retention | **APPROVED — no automatic deletion.** Turning Dev mode off deletes nothing; the §6 size/count caps are the only *automatic* deletion path. **SUPERSEDED — see docs/contracts/P91-F6-usage-retention.md §3.3.** Age-based pruning IS automatic deletion **of metrics**; the §6 prohibition on automatic **log** deletion is untouched. | §6, §10, §12 inc. 1 & 7 |
| 5 | Log file granularity | **APPROVED — one file per session** | §6 |
| 6 | `metrics_reset` | **APPROVED — ship the command, expose no UI** until a Statistics page exists. **SUPERSEDED — see docs/contracts/P91-F6-usage-retention.md §5.** | §6, §10, §12 inc. 6 |
