...
**Owner:** architect · **Written:** 2026-09-11 · **Status:** contract, not yet implemented
...
> ~1973 lines — **3.3× the ~600-line lean-contract cap** — and the standing instruction is to split
...
> passages in place means re-emitting all 1973 lines and risking the silent loss of one of its 33
> ratified decision rows. **This file is normative and overrides every sentence it quotes as
> SUPERSEDED.** §2 lists the exact one-line pointers an Edit-capable agent (senior-dev or
> docs-curator) must splice in; until they are spliced, a reader of that file alone will read stale
> text — that is the accepted cost and it is flagged in §10.
...
## 2. Passages superseded (ten), and the pointers to splice
...
/// Set when `prune_days` folded at least one bucket at load; cleared by the first
/// `persist()` that commits. While true, `usage.json.bak` may hold pre-window data.
bak_stale: bool,
```
...
clear(perf, now_secs, mode):
    ...
    (path, fresh, rev) = {
        ...
        g.bak_stale = false                 # the old .bak is handled explicitly below
        mark_dirty(&mut g)                  # bumps `rev` — this is the fence token
        g.dirty = false                     # nothing scheduled may re-write the old data
        (g.path.clone()?, g.file.clone(), g.rev)   # no path (uninitialised) => Ok(default counts)
    }
    ...
    counts = match mode:
        ResetInPlace => { guard.commit(&path, &fresh)?;
                          remove_file(bak_path(&path));   # else the PRE-clear profile lives on
                          MetricsClearCounts{ dir_removed: false, .. } }
        DeleteFiles  => purge_metrics_dir(path.parent()?) # removes usage.json{,.bak,.tmp} + the dir
    drop(guard)
    Ok(counts)
```

**Why the `commit_rev` fence, and what happens to an in-flight writer** (the orchestrator's
question). Three cases, all closed by the two steps above and by machinery §8.3 already ratified:
...
pub async fn logs_delete_all(
    ...
    perf: tauri::State<'_, Arc<PerfState>>,         // NEW — for the §4.2 re-baseline
) -> Result<LogsDeleteResult, AppError>;

#[tauri::command]
pub async fn metrics_reset(
    metrics: tauri::State<'_, Arc<MetricsState>>,
    perf: tauri::State<'_, Arc<PerfState>>,         // NEW
) -> Result<(), AppError>;
```
...
| File | Change |
|---|---|
| `src-tauri/src/obs/metrics.rs` | `RETAIN_DAYS = 90`; `prune_days`; `Inner.bak_stale`; `clear(..)` + `ClearMode` + `MetricsClearCounts`; call `prune_days` in `init` and `totals_for`; doc comments |
| `src-tauri/src/obs/metrics_purge.rs` | **NEW** — `purge_metrics_dir` (§4.3) |
...
| `src-tauri/src/obs/metrics_file.rs` | make `bak_path` visible to `metrics.rs` / `metrics_persist.rs` if it is not already |
| `src-tauri/src/obs/metrics_keys.rs`, `metrics_map.rs` | doc comments: keep the privacy rationale, drop "un-deletable" (§2) |
| `src-tauri/src/commands/obs.rs` | both command signatures + the merged counts (§6) |
...
| `src-tauri/src/obs/tests_metrics.rs`, `tests_purge.rs` | §8. `tests_purge.rs:78`/`:92` currently assert `metrics/usage.json` **survives** — that assertion **inverts** |
...
9. `logs_delete_all` with Dev mode **ON** and **OFF** both return `metricsCleared: true`, and
   `deletedFiles` includes `deletedMetrics`. `tests_purge.rs`'s "metrics untouched" assertion is
...
- **F6-A — `P91-observability.md` still reads stale until the §2 pointers are spliced.** That file is
  ~1973 lines, 3.3× the lean-contract cap, and the standing instruction is to split rather than grow
  it; re-emitting all of it from a `Write`-only role to change ten passages also risks dropping one
  of its 33 ratified decision rows. **Ask senior-dev (or docs-curator) to insert one
  `**SUPERSEDED — see docs/contracts/P91-F6-usage-retention.md §N.**` line at each of the ten §2
  locations**, and to append §11's three rows to §13. That is a mechanical, low-risk edit for an
  agent with `Edit`; it is not one for an agent that must rewrite the whole file.
- **F6-B — the shipped `reset()` has a data-resurrection defect** (§4.2, `perf_baseline` zeroed while
  `PerfState` is process-lifetime). It is currently unreachable from any UI, which is why nobody has
  seen it, and the new delete action would have inherited it. Fixed by the shared primitive.
- **F6-C — `metrics_reset` gains a state parameter** (`PerfState`). It is UI-less, so no user-visible
  change, but its call sites and tests move.
- **F6-D — `tests_purge.rs:78`/`:92` assert the opposite of the ruling.** They are the tests that
  pinned "metrics/ untouched". Their inversion is the single clearest signal that the scope changed
  deliberately, so keep them (inverted) rather than deleting them.
...
## 11. Rows to append to `P91-observability.md` §13
