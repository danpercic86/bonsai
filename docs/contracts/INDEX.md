...
| `P113d-watcher-log-volume.md` | P113d | Watcher log volume — an amendment to `P91-observability.md` §2.4 / §3.1, Rust only. 10,280 watcher records were 87% of a 2.2 MB / 6-minute log and 7,247 of them had `relevant: 0`. **`DEBOUNCE` (300 ms) is explicitly NOT touched** — 8,384 relevant batches coalescing into 192 fires is 44:1, so the debounce works and the defect is purely in logging. No IPC command/event/channel change, no frontend behaviour change, no mock change, not a UI change; one TS type file gains five optional fields mirroring the Rust record. Preserves `P110-watcher-burst-scoping.md` in full. **436 lines.** | **rev 2, ready for senior-dev, NOT started** — rev 1's evidence tally was wrong and §0 records the correction. P113's board entry says "P113d contract rev 2 approved and ready to implement; not started". Its review is what spun out **P116** |
...
| `P115-stale-repo-paths.md` | P115 | Stale repo paths in `settings.json`: prune `recentRepos` + `hooksAckRepos` entries whose folder is provably gone, migrate (never delete) a dead `repoForgeOverrides` pin. Root-reachability + NotFound-ancestor gate so an unmounted drive or a dead share is never pruned. New `settings/prune.rs`, one-shot at app `setup`. | **rev 2 (2026-09-22)** — rev 1 implemented + reviewed (approve, zero MUST-FIX); rev 2 amendments from the security audit are spec-only so far. Changed sections carry a **[REV 2]** marker and are confined to §1.3/§4/§5/§9/§10; §1.1-§1.2 verdicts, the §3 gate, the §6 call site and §7 log-only reporting are confirmed and closed. Rev 2 adds migration **rule (6)** (positive forge-host match — a cross-host pin is invisible, unremovable and permanent, not inert), the dead-side-`norm` / live-side-canonical comparison split, a `#[cfg(windows)]` gate on the case fold, and root-first probing. **No IPC change, no UI surface** (`ui-designer` not required). Version stays 1. §10 lists four deliberate omissions incl. the `pinnedBy` marker that closes the accepted same-host residual |
| `P117-perf-signal-fixes.md` | P117 | Two independent 2026-09-22 perf signals, specced as two standalone increments. **Inc 1:** the per-repo graph cache was wiped on every `full` refresh round because `open_repo_inner` has no early return for an already-open repo — `open_repo_inner` now carries the existing `Arc<GraphCache>` over under the same lock acquisition as the insert. **Inc 2:** the `redundant-refresh` and `cache-collapse` anomaly rules were repo-blind (`RefreshPayload` carried no repo id), so unrelated repos were merged — a `repo` dimension is added to `LogRecordBase` as one optional field, keyed `(repo, scope)`, with a writer-side field-name redaction rule in `strict::enforce`. `OBS_SCHEMA_VERSION` **2 → 3** (changed *meaning* of existing `anomaly` records, not an additive field). **617 lines** (measured 2026-09-23; the board's "583" predates `91d9377`). | **implemented, both increments; `awaiting USER CHECKPOINT`** — `d63a571` (inc 1) + `83bbdf3` (inc 2), reviewer approved with no MUST-FIX, `security-auditor` CLEAN at HIGH and above, AI gate green (9/9, 561.2s). Not `done`: two checkpoint items need the native app and real repos. **Four contract-record corrections are owed to `architect`** (§1.2's `info.path`-is-canonical claim, AC1-4's unimplementable construction, §1.2's `same_repo_path` fallback branch, and AC2-9's scope) — live in `TODO.md`. Companion audit: `docs/audit-2026-09-22-perf-signals.md`, which carries a **CORRECTION block**, so do not restate its original 17-of-26 cross-repo split as exact |
...
> **Add to that list, 2026-09-23: P116 and P118/P118b.** Both shipped without a contract file —
> P116 was a four-file spun-out fix (`e49cf20`), P118/P118b are the sidebar render-storm work
> (`8790b62`, `c6ae304`). Board-only, recorded here so no session greps for a file that does not
> exist. P116 is archived (`docs/history/todo-archive-2026-09.md` Part 83); P118/P118b are live.

> **Curator note, 2026-09-23.** **Two files had no row and are added:** `P113d-watcher-log-volume.md`
> and `P117-perf-signal-fixes.md`. P117's absence was flagged by the P117 session itself
> (`bbd8993` added P115's row and carried no foreign hunks; P117's was simply never written);
> P113d's was not flagged by anyone. **Every file in `docs/contracts/` now has a row** — verified by
> iterating the directory, not by eye. **No status was changed this pass**, and nothing was moved
> to `archive/` (`git mv` is outside the curator's file allowlist; the archive-eligible backlog from
> the 2026-09-22 note stands unchanged). **One measured drift recorded rather than left implicit:**
> `P117-perf-signal-fixes.md` is **617 lines**, not the 583 the board states — it grew in `91d9377`,
> the commit that corrected four wrong statements in it.
