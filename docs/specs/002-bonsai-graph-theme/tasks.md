...
- [ ] 1. Palettes + Theme extension: create `src/graph/palettes.ts` (move existing standard
...
- [ ] 2. Settings wiring + persistence: add `graphStyle: 'standard' | 'bonsai'` (default
...
- [ ] 3. Bonsai painters: branch node + edge + backdrop drawing on the Theme flags. Nodes keep the
  ...
  owner: senior-dev
- [ ] 4. Settle-on-scroll sway: `src/graph/sway.ts` (bounded settle offset, ≤1px, node glyphs
...
- [ ] 6. Tests: unit (`resolveTheme()` returns correct palette+flags for each
...
- **Invariant:** this is React-render-only. No Rust, no graph-layout math, no new IPC command — the
  settings ride the existing `setUiSettings`/`getUiSettings` surface. A reviewer finding of any
  Rust/topology change is a MUST-FIX.
