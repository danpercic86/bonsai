...
**Status:** done (AI gate green 2026-08-27; rail feel = USER CHECKPOINT pending)

- [x] 1. Frontend unit (single senior-dev pass — frontend-only feature): `src/graph/rail/*` (OverviewRail component, bucket build/resample math as pure modules, RailRowModel seam), `src/graph/drawRail.ts` painter, `src/styles/graph-rail.css`, GraphCanvas integration (≤~25 ln, hover zone in existing mousemove, unmount-when-hidden), `currentMatchRow` derivation, `graphMinimapAlwaysShow` pref chain + `SettingsGraphOverviewSection.tsx` (Commit-graph "Overview" group), mock persistence; extract `railProps.ts` if WorkspaceGraphPane nears 500 ln; vitest for bucket/coalesce/thumb/rowmodel math — owner: senior-dev
- [x] 2. Review diff (perf: zero idle cost hidden, no per-paint allocs; correctness of mapping math) + design review (contract fidelity, both themes/styles) — owner: reviewer + ui-designer (concurrent)
- [x] 3. MUST-FIX round(s) — owner: senior-dev
- [x] 4. Tests + e2e smoke (hidden by default + no .graph-rail in DOM, search-open reveal + tick click-to-jump + current-match emphasis, always-show persistence, click/drag thumb jump) — owner: tester
- [x] 5. Orchestrator: gate, harness proof, commit `wip(spec-005)`, TODO ledger — owner: orchestrator
