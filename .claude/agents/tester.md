...
- Graph performance: maintain a scripted generator for a synthetic 20k+ commit fixture repo
  (use git2 or `git fast-import` — never thousands of individual `git commit` calls) and a
  criterion benchmark for the graph-layout engine; the budget is layout < 500 ms for 20k
