...
//! autostash of a dirty tracked worktree, stash retention on conflict, and the
//! stash-pop-conflict outcome. The editable cherry-pick message (incl. its
//! survival across a conflict pause via MERGE_MSG) lives in
//! `essentials_message_cli.rs`, split out to keep each file under the
//! ~500-line limit.
//!
...
// The editable custom-message cases live in their own file (~500-line limit),
// declared here as a child module so they share `require_git!` and the
// twin-repo fixtures above.
#[path = "essentials_message_cli.rs"]
mod essentials_message_cli;
