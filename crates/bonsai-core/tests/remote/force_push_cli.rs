...
use crate::common;
use crate::common::{commit_fixed, git, git_env};
...
fn lease_refuses_when_remote_moved() {
    ...
    assert_eq!(
        origin_main(&f),
        y,
        "origin main must be unchanged after refusal"
    );
}
...
fn lease_succeeds_and_moves_the_ref() {
    ...
    assert_eq!(
        origin_main(&f),
        z,
        "origin main must move to the rewritten tip"
    );
}
...
fn up_to_date_when_baseline_equals_local_tip() {
    ...
    let res =
        force_push_with_lease(&f.work, &PanicExec, false).expect("up-to-date is not an error");
    match res {
...
fn no_upstream_is_rejected() {
    ...
    let err =
        force_push_with_lease(&f.work, &PanicExec, false).expect_err("no upstream must error");
    assert!(
...
fn no_baseline_refuses_with_fetch_hint() {
    ...
    let err =
        force_push_with_lease(&f.work, &PanicExec, false).expect_err("no baseline must refuse");
    match err {
        AppError::PushRejected(m) => {
            assert!(
...
fn lease_succeeds_on_nested_branch_name() {
    ...
    let res = force_push_with_lease(&f.work, &SpawnGitExec, false)
        .expect("lease should hold on nested branch");
    match res {
        PushResult::Pushed {
            remote,
            branch,
...
fn force_push_drops_a_remote_commit() {
    ...
    assert_eq!(
        origin_main(&f),
        z,
        "origin main must be the replacement tip Z"
    );
    ...
    assert!(
        !reachable,
        "dropped commit Y must not be an ancestor of origin/main"
    );
}
...
fn detached_head_is_rejected() {
    ...
    let err =
        force_push_with_lease(&f.work, &PanicExec, false).expect_err("detached HEAD must error");
    match err {
...
// The §A6 pre-push hook oracle lives in its own file (~500-line limit),
// declared here as a child module so it shares `require_git!` and the fixture
// helpers above.
#[path = "force_push_hooks_cli.rs"]
mod force_push_hooks_cli;
