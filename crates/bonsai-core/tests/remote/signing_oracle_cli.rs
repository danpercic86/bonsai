//! CLI-oracle integration tests for commit signing + verification (P58), ORACLE
//! tier — split out of `signing_cli.rs` to keep each file under the ~500-line
//! limit. The PURE tier (config resolution, wire shapes, degrade) stays there.
//!
//! Guarded by `have_git()` / `have_ssh_keygen()`: drives the real `git` binary
//! against a scratch repo — SSH signing is hermetic (an ephemeral ed25519 key
//! with an EMPTY passphrase needs no agent). GPG is the USER CHECKPOINT.
//! `TMP`/`TEMP=D:\Data\Temp` via `common::scratch_dir`.
...
use bonsai_core::git::signing::{
    signing_status, verify_commits, CommitVerification, SignFormat, VerifyResults, VerifyStatus,
};
