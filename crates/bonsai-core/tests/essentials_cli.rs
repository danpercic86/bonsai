...
fn essentials_1_amend_matches_cli() {
    ...
    amend_commit(a, "amended subject", None, false).expect("bonsai amend");
    git_env(
        b,
...
fn essentials_1b_message_only_amend_matches_cli() {
    ...
    amend_commit(a, "reworded", None, false).expect("bonsai amend");
    git_env(
        b,
...
fn essentials_1c_merge_amend_preserves_both_parents() {
    ...
    git(d, &["add", "extra.txt"]);
    amend_commit(d, "merge topic (amended)", None, false).expect("amend merge");
