...
mod tests {
    ...
    fn argv_tools_matches_the_policy_verbatim() {
        ...
        assert_eq!(
            value_after(&read_only, "--tools").as_deref(),
            Some("Read,Grep,Glob")
        );
        let none = argv(
            PROMPT,
    ...
    fn argv_budget_is_four_decimals_and_absent_when_unset() {
        ...
        assert!(
            !unset.iter().any(|a| a == "--max-budget-usd"),
            "argv: {unset:?}"
        );
        let set = argv(
            PROMPT,
        ...
        assert_eq!(
            value_after(&set, "--max-budget-usd").as_deref(),
            Some("0.5000")
        );
        let tiny = argv(
            PROMPT,
            &RunOpts::default(),
            &RunLimits {
                max_budget_usd: Some(0.00004),
                ..interactive()
            },
        );
        assert_eq!(
            value_after(&tiny, "--max-budget-usd").as_deref(),
            Some("0.0000")
        );
    }
    ...
    fn argv_partial_messages_flag_is_opt_in() {
        ...
        assert!(
            !off.iter().any(|a| a == "--include-partial-messages"),
            "argv: {off:?}"
        );
        let on = argv(
            PROMPT,
        ...
        assert!(
            on.iter().any(|a| a == "--include-partial-messages"),
            "argv: {on:?}"
        );
    }
    ...
    fn argv_carries_the_system_prompt_only_when_set() {
        ...
        assert!(
            !none.iter().any(|a| a == "--append-system-prompt"),
            "argv: {none:?}"
        );
        let opts = RunOpts {
        ...
        assert_eq!(
            value_after(&some, "--append-system-prompt").as_deref(),
            Some("be terse")
        );
    }
