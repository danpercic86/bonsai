...
use crate::graph::{graph_seed, stream_graph_core, GraphChunk, GraphFilter};
...
fn graph_seed_on_unborn_repo_is_ok_and_empty() {
    ...
    let seed = graph_seed(dir.path(), &GraphFilter::default())
        .expect("graph_seed must return Ok on an unborn repo");
...
fn stream_graph_on_unborn_repo_emits_meta_then_empty_done() {
    ...
    match &chunks[1] {
        GraphChunk::Done {
            ...
            assert!(
                fold_spans.is_empty(),
                "an unborn repo has no rows, so there is nothing to fold"
            );
        }
