...
use crate::git::hooks::{run_hook_streaming, HookName};
use crate::git::stage::open_workdir_repo;
// P87: the activity-recording cores live in `commit_activity` (file-size split);
// re-exported so `crate::git::commit::create_commit_with_activity` keeps
// resolving for the command layer.
pub use crate::git::commit_activity::{amend_commit_with_activity, create_commit_with_activity};
...
pub fn create_commit(
    ...
    create_commit_with_activity(workdir, message, sign, skip_hooks, None)
}
...
pub(crate) fn branch_shorthand_after(
...
pub fn amend_commit(
    ...
    amend_commit_with_activity(workdir, message, sign, skip_hooks, None)
}
