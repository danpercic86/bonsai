...
fn pr_diff_is_three_dot_from_merge_base() {
    ...
    assert_eq!(
        paths,
        vec!["file_feat.txt"],
        "only the PR's own change: {paths:?}"
    );
...
fn pr_diff_unrelated_histories_empty_merge_base() {
    ...
    assert!(stats
        .files
        .iter()
        .any(|f| f.path == "b.txt" && f.status == FileStatus::Added));
...
fn pr_diff_matches_git_three_dot_ground_truth() {
    ...
    let stats = pr_diff_headers(p, &base, &head).expect("pr diff");
    assert_eq!(
        stats.merge_base_oid, o.oid,
        "merge-base is the fork point O"
    );
    assert_eq!(
        stats.additions, git_adds,
        "additions vs git three-dot\n{numstat}"
    );
    assert_eq!(
        stats.deletions, git_dels,
        "deletions vs git three-dot\n{numstat}"
    );
    assert_eq!(
    ...
    assert!(
        engine_files.contains("newname.txt"),
        "rename target present"
    );
...
fn pr_diff_binary_file_is_flagged() {
    ...
    assert!(
        numstat
            .lines()
            .any(|l| l.starts_with("-\t-\t") && l.ends_with("blob.bin")),
