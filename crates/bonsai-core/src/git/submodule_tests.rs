...
fn info_serializes_camel_case_keys() {
    let info = SubmoduleInfo {
        ...
        abs_path: Some("/repo/vendor/libcore".to_string()),
...
/// A UNC `.gitmodules` path — `Path::join` DISCARDS the base entirely, so an
/// unchecked `abs_path` would dial an attacker host on `p.exists()`. MUST be
/// `None`. Both `//host/share` and `\host\share` forms.
#[test]
fn list_submodules_nulls_unc_path() {
    for raw in ["//attacker.example/share", r"\attacker.example\share"] {
        let dir = super_with_gitmodules(raw);
        let row = only_submodule_row(dir.path());
        assert_eq!(
            row.abs_path, None,
            "a UNC .gitmodules path {raw:?} must yield abs_path: None"
        );
    }
}
