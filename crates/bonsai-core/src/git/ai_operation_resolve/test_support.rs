//! Shared git2 fixtures for the `ai_operation_resolve` test modules. Extracted
//! verbatim from the former inline `mod tests`; `pub(super)` so both sibling
//! test modules reuse them.
...
pub(super) fn expect_proposed(o: PlanOutcome) -> ProposedOperation {
    match o {
