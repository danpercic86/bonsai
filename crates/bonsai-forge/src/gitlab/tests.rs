...
fn viewer_maps_get_user_username() {
    let (p, seen) = provider_spy(
        ...
        vec![(
            "/user",
            200,
            r#"{ "username": "tanuki", "avatar_url": "https://a/t.png" }"#,
        )],
    );
...
fn list_prs_caps_per_page_encodes_id_and_maps_iid() {
    ...
    assert!(
        reqs[0]
            .url
            .contains("/projects/group%2Fsub%2Fproj/merge_requests"),
        "url: {}",
        reqs[0].url
    );
...
fn merge_pr_puts_squash_flag_and_remove_source_branch() {
    ...
    let (p, seen) = provider_spy(
        Some("glpat"),
        vec![("/merge_requests/7/merge", 200, MR_DETAIL)],
    );
    ...
    assert!(
        reqs[0].url.contains("/merge_requests/7/merge"),
        "url: {}",
        reqs[0].url
    );
    let sent: serde_json::Value = serde_json::from_str(reqs[0].body.as_ref().unwrap()).unwrap();
...
fn merge_pr_rejects_rebase_and_fast_forward_without_sending() {
    ...
    for m in [MergeMethod::Rebase, MergeMethod::FastForward] {
        ...
        assert!(matches!(
            p.merge_pr(7, &merge_input(m)),
            Err(AppError::ForgeApi(_))
        ));
        assert!(
            seen.lock().unwrap().is_empty(),
            "nothing sent for unsupported method {m:?}"
        );
    }
}
...
fn merge_pr_not_mergeable_maps_to_forge_api() {
    ...
    for status in [405, 406, 409] {
        let p = provider(
            Some("glpat"),
            vec![("/merge_requests/7/merge", status, "{}")],
        );
        match p.merge_pr(7, &merge_input(MergeMethod::Merge)) {
            Err(AppError::ForgeApi(m)) => {
                assert!(m.contains("not mergeable"), "status {status}: {m}")
            }
