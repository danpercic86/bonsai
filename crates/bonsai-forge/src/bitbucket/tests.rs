...
fn merge_pr_posts_merge_strategy_and_close_source_branch() {
    ...
    let (p, seen) = provider_spy(
        Some("bb-tok"),
        vec![("/pullrequests/7/merge", 200, PR_MERGED)],
    );
    ...
    assert_eq!(reqs[0].method, HttpMethod::Post);
    assert!(
        reqs[0].url.contains("/pullrequests/7/merge"),
        "url: {}",
        reqs[0].url
    );
    let sent: serde_json::Value = serde_json::from_str(reqs[0].body.as_ref().unwrap()).unwrap();
...
fn merge_pr_not_mergeable_maps_to_forge_api() {
    ...
    for status in [400, 409] {
        let p = provider(
            Some("bb-tok"),
            vec![("/pullrequests/7/merge", status, "{}")],
        );
...
fn close_pr_posts_decline_with_empty_body() {
    let (p, seen) = provider_spy(
        Some("bb-tok"),
        vec![("/pullrequests/7/decline", 200, PR_DECLINED)],
    );
    ...
    assert_eq!(reqs[0].method, HttpMethod::Post);
    assert!(
        reqs[0].url.contains("/pullrequests/7/decline"),
        "url: {}",
        reqs[0].url
    );
    let sent: serde_json::Value = serde_json::from_str(reqs[0].body.as_ref().unwrap()).unwrap();
    assert!(
        sent.as_object().unwrap().is_empty(),
        "decline sends an empty body"
    );
}
