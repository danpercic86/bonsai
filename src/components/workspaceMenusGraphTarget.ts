...
export function refItems(
  ...
  return [
    ...
    ...commitItems,
    // Spec-003 §3.1: the checked-out branch gets the solo/hide group too
    // (hiding it only drops its pill — the backend always keeps HEAD's
    // ancestry). Its own reset items are [] for HEAD's tip, so the group
    // still sits below everything else.
    ...b.deps.refFilterItems(`refs/heads/${r.name}`, 'branch'),
  ];
}
