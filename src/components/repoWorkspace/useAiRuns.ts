...
import { settleRun } from './aiRunSettle';
import {
  conflictKey,
  deriveRowStates,
  isTerminalStatus,
...
export function useAiRuns(deps: AiRunsDeps): AiRunsApi {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Audit §3.10: closing the tab must not leave the Claude CLI running (and
      // spending) with no dock or cancel affordance — cancel every live run.
      // Fire-and-forget: cleanup cannot await, and a failed cancel has no
      // recipient left to tell. StrictMode-safe: the synthetic dev cleanup runs
      // immediately after mount, while `runsRef` is still empty (runs only start
      // from user callbacks), so nothing is cancelled spuriously.
      for (const run of Object.values(runsRef.current)) {
        if (!isTerminalStatus(run.status) && run.runId !== null) {
          void ipc.aiCancelRun(run.runId).catch(() => undefined);
        }
      }
    };
  }, []);
  ...
  /** The batch → per-file-state routing (and its markerful SAFETY GATE) lives in
   *  `aiRunSettle.ts` (file-size discipline). Every ctx field is a stable ref or
   *  stable callback, so the identity of `settle` never churns. */
  const settle = useCallback(
    (key: string, batch: AiResolveBatch) =>
      settleRun({ runsRef, depsRef, mounted, slotAtStart, patch, flush }, key, batch),
    [flush, patch],
  );
