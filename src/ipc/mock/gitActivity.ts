 ...
 *   ?gitConflicts  — a SUCCESSFUL, classified run of a conflict-capable
 *                    category (`MOCK_CONFLICT_CAPABLE`) ends `outcome: 'conflicts'`.
 *   ?gitMultiTarget — `deleteBranches` / `discard` start with `targetCount: 3`.
 *   ?gitLongTarget also covers a 90+-char submodule name and discard path.
 *   (`?gitOpFail` lives in the checkoutBranch handler and `?refreshSlow` in
 *   getStatus: both must fire with no subscriber attached.)
 *
 * Target/line funnels + the H/R/C resolvers: `gitActivityTargetArg.ts`;
 * outcome classifiers: `gitActivityOutcome.ts`.
 */
import { MOCK_PRE_PUSH_OUTPUT } from './hooksGate';
import { MOCK_CONFLICT_CAPABLE } from './gitActivityOutcome';
import {
  MAX_ACTIVITY_LINE_CHARS,
  mockActivityLine,
  mockReasonLine,
  stripActivityControls,
  truncateActivityChars,
...
const GIT_CONFLICTS = query('gitConflicts') !== null;
const GIT_MULTI_TARGET = query('gitMultiTarget') !== null;
...
/** `?gitLongTarget` on the submodule rows — a 90+-char submodule NAME (a
 *  `Name`, so it may be nested) whose leaf survives the ellipsis. */
export const MOCK_LONG_SUBMODULE_TARGET =
  'vendor/third-party/platform-integrations/observability/exporters/opentelemetry-collector-contrib';

/** `?gitLongTarget` on `discard` — a 90+-char file path (the single-file case). */
export const MOCK_LONG_PATH_TARGET =
  'src/components/repoWorkspace/very/deeply/nested/feature/folder/structure/GitActivityDockRow.tsx';
 ...
 *
 * The strip + cap are the shared `stripActivityControls` / `truncateActivityChars`
 * mirrors (one funnel for targets and lines, as in Rust).
 */
export function mockActivityTarget(raw: string | null): string | null {
  if (raw === null) return null;
  const clean = stripActivityControls(raw).trim();
  if (clean === '') return null;
  return truncateActivityChars(clean, MAX_ACTIVITY_TARGET_CHARS);
}
...
function seamTarget(category: GitActivityCategory, target: string | null): string | null {
  ...
  if (GIT_LONG_TARGET && category.startsWith('submodule')) return MOCK_LONG_SUBMODULE_TARGET;
  if (GIT_LONG_TARGET && category === 'discard') return MOCK_LONG_PATH_TARGET;
...
class GitSequencer {
  ...
  /** Every line goes through the `activity_line` mirror (controls stripped,
   *  2000-char cap), exactly like the real emitter. */
  stdout(line: string): void {
    this.emit('stdoutLine', { line: mockActivityLine(line) });
  }
  stderr(line: string): void {
    this.emit('stderrLine', { line: mockActivityLine(line) });
  }
   ...
   *  message (line breaks → spaces, then the line funnel), then `finished`.
   *  Skipped for `hookRejected` (its hook output is already on the stream).
   *
   *  A plain thrown `Error` (e.g. stash's `guardExpectedOid`) ALSO gets the
   *  line: in the real backend every command error is an `AppError` whose
   *  `message` is exactly that text, and contract §6 pins "a throwing handler
   *  emits the message line". Anything else thrown (not an object with a
   *  message) has no user-facing text, so no line. */
  failed(e: unknown): void {
    const reason = isAppError(e)
      ...
      : e instanceof Error
        ? e.message
        : null;
    if (reason !== null) this.emit('stderrLine', { line: mockReasonLine(reason) });
...
async function runPlain<T>(
  ...
  // ?gitMultiTarget — the two multi-item rows start with a count of 3.
  const multi = category === 'deleteBranches' || category === 'discard';
  s.start(category, target, GIT_MULTI_TARGET && multi ? 3 : opts.count);
  try {
    ...
    const result = await fn();
    // Only a run that HAS a classifier: `createBranch` is capable via
    // createBranchHere's stash re-apply, never via the plain create.
    const forced = GIT_CONFLICTS && opts.classify !== undefined && MOCK_CONFLICT_CAPABLE.has(category);
    s.finished(0, true, forced ? 'conflicts' : (opts.classify?.(result) ?? null));
    return result;
  } catch (e) {
    s.failed(e);
    throw e;
